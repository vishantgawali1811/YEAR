...
via risk_scoring.py, and remediation guidance -- rule-based steps with an
optional Groq-generated detailed explanation layered on top).
"""
...
from exploit_recommender import recommend_exploits
...
class AgentResponse(BaseModel):
    ...
    exploit_recommendations: list[dict] = []
    ...
    def run(self, query: str) -> AgentResponse:
        ...
        if not product or not version:
            return AgentResponse(
                ...
                results=[], exploit_recommendations=[],
            )
        ...
        for _, row in ranked_df.iterrows():
            results.append(AgentResultItem(
                ...
                remediation_detail=remediation_detail_map.get(row["cve_id"]),
            ))
        ...
        return AgentResponse(
            ...
            exploit_recommendations=recommend_exploits(product, version),
        )
        ...
        for r in response.results:
            ...
            if r.remediation:
                for step in r.remediation:
                    print(f"    - {step}")
            if r.remediation_detail:
                print(f"    DETAIL: {r.remediation_detail}")