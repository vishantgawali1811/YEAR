...
export default function Dashboard({ stats }) {
  ...
  return (
    <div>
      ...
      {searched && (
        <div className="terminal-box" style={{ padding: 'var(--sp-5)', marginTop: 'var(--sp-5)' }}>
          <div className="terminal-box-title">// PUBLIC_EXPLOIT_RECOMMENDATIONS</div>
          {exploitRecommendations.length === 0 ? (
            <div style={{ color: 'var(--text-muted)', fontSize: 'var(--fs-sm)', paddingTop: 8 }}>
              No relevant public exploits found for this technology and version.
            </div>
          ) : (
            <div style={{ overflowX: 'auto', paddingTop: 8 }}>
              <table className="cve-table" aria-label="Public exploit recommendations">
                <thead>
                  <tr><th>RANK</th><th>EDB ID</th><th>DESCRIPTION</th><th>PLATFORM</th><th>TYPE</th><th>PORT</th><th>RELEVANCE</th><th>WHY</th></tr>
                </thead>
                <tbody>
                  {exploitRecommendations.map((exploit, index) => (
                    <tr key={`${exploit.id}-${index}`}>
                      <td>{index + 1}</td>
                      <td style={{ color: 'var(--primary)', fontWeight: 700 }}>EDB-{exploit.id}</td>
                      <td style={{ minWidth: 280 }}>{exploit.description}</td>
                      <td>{exploit.platform || '—'}</td>
                      <td>{exploit.type || '—'}</td>
                      <td>{exploit.port || '—'}</td>
                      <td style={{ color: 'var(--risk-low)', fontWeight: 700 }}>{Math.round(exploit.relevance_score * 100)}%</td>
                      <td style={{ minWidth: 280, color: 'var(--text-muted)', fontSize: 'var(--fs-xs)' }}>
                        {exploit.reasons?.map((reason, reasonIndex) => <div key={reasonIndex}>✓ {reason}</div>)}
                      </td>
                    </tr>
                  ))}
                </tbody>
